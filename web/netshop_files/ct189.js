...
s_jt.codeVersion='20150915';
s_jt.usePlugins=true
if(window.location!= window.top.location){s_jt.t=function(){};}
...
s_jt.trackingServer = "183.56.130.232"
s_jt.t();
...
function trkInternalSearch(){
	var key = document.getElementById('sv');
    if(key && key.value) {
		s_jt.eVar2 = decodeURI(key.value);
...
function checkSearchResult(){
	...
	if(result_div){
		...
		if(search_result && !isNaN(search_result)){
			...
			var key = document.getElementById('sv');
			if(key && key.value) {
				s_jt.eVar37 = decodeURI(key.value); 
			}
			s_jt.eVar38 = "y";
			if(search_result == "0"){
...
function trkSearchResultClick(obj){
	if(s_jt.eVar2){
		var div = document.getElementById("pinpai1");
		if(isParent(obj, div)){
...
function trkCitycode(){
	...
	var cmpid = s_jt.Util.getQueryParam('cmpid');
	...
	var intid = s_jt.Util.getQueryParam('intid');
...
function trkBaseTrafficInfo() {
    ...
    s_jt.eVar1 = 'D=vid';
    s_jt.eVar3 = s_jt.Util.getQueryParam('intid');
	s_jt.eVar39 = s_jt.Util.getQueryParam('intaid');
    s_jt.eVar8 = window.trkPayType;
    s_jt.eVar9 = window.trkBank;
	s_jt.prop41 = window.sfjc;
    s_jt.campaign = s_jt.Util.getQueryParam('cmpid');
...
function trkMouseList(){
	 trkXiaoyuanArea();
	 trkXiaoYuanSerchButton();	
}
...
function trkXiaoyuanArea(){
	if(doPluginsFlag == 1 && s_jt.pageName=="/xiaoyuan/"){
		var btn = getFirstElementByAttr("button", "class", "bt_01");
		if(btn){
			addEvent(btn, "click", function(evt){
			var provice=document.getElementById("provice_dt").innerHTML;
			var city=document.getElementById("city_dt").innerHTML;
			if((provice&& provice!="省份" )&& (city &&city!="城市")){					
			s_jt.linkTrackVars="eVar4,prop35,prop37";
			s_jt.prop35=decodeURI(provice);
			s_jt.prop37=decodeURI(city);
			s_jt.tl(true, 'o', "trkXiaoyuanArea"); 
			}
		});		
		}		
	}
...
function trkXiaoYuanSerchButton(){
	if(doPluginsFlag == 1 && s_jt.pageName=="/xiaoyuan/"){
		var btn=getFirstElementByAttr("button","class","ss_01 fl");
		if(btn){
			addEvent(btn, "click", function(evt){			
			var keyWord=document.getElementById("Text2");				
			s_jt.linkTrackVars="eVar4,prop36";				
			if(keyWord){	
	       	...
	       	s_jt.tl(true, 'o', "trkXiaoYuanSerchButton"); 	
		});	
		}		
	}	
}	
